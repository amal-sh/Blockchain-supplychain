...
const getHistoryIcon = (details) => {
    if (!details) return <History className="w-5 h-5 text-gray-600" />;
...
const Timeline = ({ events, actorProfiles, title }) => {
    ...
    // Sort events to handle the "Split" swap and maintain chronological order
    const sortedEvents = [...events].sort((a, b) => {
        const timeA = Number(a.timestamp);
        const timeB = Number(b.timestamp);
        if (timeA !== timeB) return timeA - timeB;

        // Robust check for "Split ... into" and "Split from"
        const isSplitFrom = (details) => details.includes("Split from");
        const isSplitInto = (details) => details.includes("Split") && details.includes("into");

        // The user wants "Split from" to appear BEFORE "Split into" if they happen together
        if (isSplitFrom(a.details) && isSplitInto(b.details)) return -1;
        if (isSplitInto(a.details) && isSplitFrom(b.details)) return 1;
                    ...
                    >
                        ...
                        <div className="bg-gray-50 p-5 rounded-lg shadow-sm border border-gray-100">
                            <div className="flex justify-between items-start mb-1">
                                <p className="text-sm text-gray-500">{new Date(Number(event.timestamp) * 1000).toLocaleString()}</p>
                                {event.productName && (
                                    <span className="text-xs font-bold text-green-700 bg-green-100 px-2 py-0.5 rounded uppercase">{event.productName}</span>
                                )}
                            </div>
    ...
    const fetchFullHistory = useCallback(async (startId, contract, existingProfiles) => {
        ...
        while (currentId > 0 && !visitedIds.has(currentId.toString())) {
            ...
            try {
                ...
                // Attach product name to each history event and ensure fields are plain values
                const historyWithContext = product.history.map(event => ({
                    actor: event.actor,
                ...
                completeHistory = [...historyWithContext, ...completeHistory];
